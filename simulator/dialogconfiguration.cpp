...
void DialogConfiguration::slotReadConfigurationLaserScanner()
{
    ...
    for(int i = 0; i < numberOfLaserScanners; ++i)
    {
        ...
        connect(newLaserScanner, SIGNAL(newLidarPoints(QVector<QVector4D>,QVector3D)), mSimulator->mBaseConnection, SLOT(slotSendScanFused(QVector<QVector4D>,QVector3D)));
