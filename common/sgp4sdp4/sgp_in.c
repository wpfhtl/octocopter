...
// ben: atof uses the current locale, so in germany, parsing
// doubles containgin a . as decimal point will not work.
// Thus, we set the locale to C and back when we're done :|
void
...
{ 
  ...
  const char* loc = setlocale(LC_NUMERIC, 0);
