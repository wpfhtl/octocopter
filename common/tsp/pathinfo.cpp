...
unsigned int MatrixInfo::reduceRow(unsigned int i, 
	const struct AdjacencyMatrix& c,	const struct PathInfo& p) 
{
...
unsigned int MatrixInfo::reduceMatrix(const struct AdjacencyMatrix& c,
...
PathInfo::PathInfo(const AdjacencyMatrix& costs) :
...
PathInfo::PathInfo(const PathInfo& old, Edge e, bool inc)
{
    ...
    next = old.next;
    foundLBAndEdge = old.foundLBAndEdge;
    bothBranches = old.bothBranches;
...
void PathInfo::setAvailAndLB(const struct AdjacencyMatrix& c, 
...
struct Path PathInfo::getTSPPath(const AdjacencyMatrix& c) const
{
...
void PathInfo::calcLBAndNextEdge(const AdjacencyMatrix& c) 
{
