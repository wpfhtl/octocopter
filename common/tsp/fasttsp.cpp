...
struct Path Graph::fastTSP(bool useOpt)
{
    ...
    while(!remaining.empty())
    {
        ...
        {
            unsigned int oldDist = this->operator()(salespath.vertices[i], salespath.vertices[i + 1]);
            unsigned int newDist = this->operator()(salespath.vertices[i], minIndex) + this->operator()(salespath.vertices[i + 1], minIndex);
    ...
    {
        salespath.length += this->operator()(salespath.vertices[i], salespath.vertices[(i + 1) % salespath.vertices.size()]);
    }
